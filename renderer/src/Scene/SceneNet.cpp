...
SceneNet::SceneNet(std::string save_base,std::string layout_file, std::string obj_base_folder,std::string layout_base_folder,int seed) {
  ...
  //Parse wnid file
  time_ = 0.0;
  std::string model_file = obj_base_folder+ "filtered_model_info_and_texture.txt";
  std::ifstream file(model_file);
  std::cout << "READING FILE: " << model_file << std::endl;
  int line_number = 1;
  std::string str;
...
optix::Group SceneNet::getSceneRootGroup(optix::Context & context) {
  ...
  {
    std::uniform_real_distribution<float> brightness(0.0,1.0);
  ...
  // Random Lights
  m_sceneLights.clear();
  std::uniform_int_distribution<> num_lights_dist(1,3);
  ...
  std::uniform_real_distribution<float> light_power_dist(0.01e2f,1.0e2); // was 2
  std::uniform_real_distribution<float> light_colour_scalar(0.5,1.0);
    ...
    if (line_number < header_point) {
      ...
      if (str == "object" || str == "end") {
        ...
        if (object_id_count != 1 || object_scale_count != 1 || object_transform_count != 3) {
          ...
          std::cout << object_id_count << " "
                    << object_scale_count << " "
                    << object_transform_count << std::endl;
          if (line_number > header_point) {
...
bool SceneNet::createOptixGeometry(aiMesh* mesh,optix::Context& context,std::vector<optix::GeometryInstance>& obj_ginstances) {
  ...
  const int num_normals = (mesh->HasNormals()) ? mesh->mNumVertices : 0;
  ...
  optix::Buffer normalBuffer = context->createBuffer( RT_BUFFER_INPUT, RT_FORMAT_FLOAT3, num_normals);
  ...
  if (num_normals > 0) {
    memcpy( static_cast<void*>( normalBuffer_Host ),
            ...
            sizeof( optix::float3 )*num_normals);
  }
  normalBuffer->unmap();
                               ...
                               std::vector<optix::GeometryInstance>& obj_ginstances)
{
  ...
  ObjectInfo layout;
  loadSceneMaterials(layout,file_base_name,m_scene,true);
...
bool SceneNet::createOBJMesh(ObjectInfo object, optix::Context& context,
  ...
  Assimp::Importer importer;
  const aiScene *m_scene = importer.ReadFile(filename,
                                             aiProcess_Debone |
  ...
  for(int mesh_id = 0; mesh_id < m_scene->mNumMeshes; mesh_id++) {
    ...
    if (!mesh->HasNormals()) {
      std::cout << "E: Mesh doesn't have normals, continuing." << std::endl;
      std::cout << "   num verts: " << mesh->mNumVertices << std::endl;
      std::cout << "   num faces: " << mesh->mNumFaces << std::endl;
      std::cout << "       prims: " << mesh->mPrimitiveTypes << std::endl;
      std::cout << "            : hypothesis: it's a animation skeleton" << std::endl;
      continue;
    } else {
      aiVector3D* normals_f3  = reinterpret_cast<aiVector3D*>( mesh->mNormals );
      /// Transform normals -> each vertex has its normal
      int num_normals = mesh->mNumVertices;
...
void SceneNet::loadSceneMaterials(const ObjectInfo& object,
                                  const std::string& model_path,
                                  const aiScene* m_scene,bool separate_materials) {
  ...
  for(unsigned int i = 0; i < m_scene->mNumMaterials; i++) {
    ...
    aiColor3D diffuseColor;
    if(material->Get(AI_MATKEY_COLOR_DIFFUSE, diffuseColor) == AI_SUCCESS) {
      auto col = toFloat3(diffuseColor);
      Material* mat_diffuse = new Diffuse(col,class_id,instance_id_);
      if (separate_materials) {
        std::string wnid = scenenetlayout_mat_to_wnid[lookup];
        log_file<<"instance:"<< instance_id_ << ";"<<wnid<<";"<<strMat<<";"<<std::endl;
        instance_id_++;
      }
      std::cout << "Added diffuse material: " << col.x << " "
                << col.y << " " << col.z << std::endl;
    ...
    if (separate_materials) {
      std::string wnid = object.wnid;
      log_file<<"instance:"<< instance_id_ << ";"<<wnid<<";"<<strMat<<";"<<std::endl;
      instance_id_++;
    }
    m_materials.push_back(mat_diffuse);
    std::cout << "Pushed red diffuse" << std::endl;
  }
  std::cout << "Num materials: " << m_materials.size() << std::endl;
