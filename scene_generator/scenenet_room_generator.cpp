...
public:
  ...
  typedef std::map<std::string,std::vector<std::string> > map_string_strings_t;
  typedef std::map<std::string,std::vector<int>> map_string_ints_t;
  ...
  {
    auto& ids = tag_to_modelid_[tag];
    size_t N = ids.size();
    if (N > 0) {
      std::uniform_int_distribution<> dist(0, N-1);
      return ids[dist(m_randgen)];
    }
  }
  ...
  void sorted_insert(V& v, const T& t) {
    auto i = std::lower_bound(v.begin(), v.end(), t);
    if (i == v.end() || t < *i) {
      v.insert(i, t); // avoid duplicates, like std::set
    }
  }
...
public:
  ...
  map_string_strings_t wnid_to_dirs_;

  // used to sample a random object from all objects with that label
  map_string_ints_t tag_to_modelid_;
...
RandomObject
get_random_object(std::mt19937& mt,
                  ObjScaling& myobjectscaling,
                  ShapeNetModel& shapenetmodel,
                  ...
                  bool largeobject)
{
  ...
  while (!found_appropriate_size) {
    ...
    if (scale < 0) {
      std::cerr << "Skipping random object: " << objectName << std::endl;
      continue;
    }
                  ...
                  std::string object_tag)
{
  ...
  int counter = 0;
  int model_id;
  std::string wnid, objectName = "NOTFOUND", model_file_path;

  while (objectName == "NOTFOUND") {
  ...
  if (scale < 0) {
    std::cerr << "Skipping random object: " << objectName << std::endl;
    throw std::runtime_error("Couldn't compute scale for " + objectName);
  }
...
add_object(ChSystem& system, RandomObject& object, ChVector<> pos, double mass = 15.0, 
           ChMatrix33<> rot = ChMatrix33<>(1), bool cog_below = true, bool fixed = false,
...
static std::string
make_filename(const std::string &base, const std::string& ext = ".txt")
{
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  if (layout_paths.size() < 1) {
    std::cerr << "Error: no layout paths from: " << p << std::endl;
    exit(-1);
  }
  
  // Pick random layout
  //std::uniform_int_distribution<> layout_dist(0,layout_paths.size()-1);
  std::uniform_int_distribution<> layout_dist(0,21); // livingroom + bedroom + office
  ...
  std::uniform_real_distribution<double> uniform_dist_xz(-0.3, 0.3);
  ...
  if (single_mode) {
    ...
    // Support modified layouts such that 0,Y/2,0 is center
    double x = 0;
    double z = 0;
    double y = bbmin.y + 0.5 * (bbmax.y - bbmin.y);
    ...
    for (int object_id = 0; object_id < num_large_objects; object_id++) {
      ...
      double x = uniform_dist_xz(mt) * (bbmax.x - bbmin.x);
      double y = bbmin.y + uniform_dist_y(mt) * (bbmax.y - bbmin.y);
      double z = uniform_dist_xz(mt) * (bbmax.z - bbmin.z);
  ...
  */
  ...
  for(auto object : objects) {
    ...
    if (mypos.x < bbmin.x || mypos.x > bbmax.x ||
        mypos.y < bbmin.y || mypos.y > bbmax.y ||
        mypos.z < bbmin.z || mypos.z > bbmax.z) {
      std::cout << "Body out of bounds!" << this_object.model << std::endl;
  ...
  if (valid <= (objects.size()/2)) {
    std::cerr << "Invalid number of objects used..." << std::endl;
    return 1;
  }
  std::cout << "Valid objects: " << valid << std::endl;
  return 0;
}
